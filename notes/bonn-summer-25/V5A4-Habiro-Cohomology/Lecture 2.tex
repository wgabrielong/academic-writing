...
\begin{proposition}\label{prop: algebra structures on RHom unit Ad and Gm}
    ...
    \begin{enumerate}[label=(\roman*)]
        \item The $q$ de Rham complex $q\dash\Omega^{\bullet}_{R/\ZZ}$ computes $R\Hom_{q\dash\Mod_{R}}(\mathbf{1},\mathbf{1})$ in the derived category of modules with $q$-connection $q\dash\Mod_{R}$ on $\spec(R)$.
        \item The $q$-Hodge complex $q\dash\Hdg_{R}$ computes $R\Hom_{q\dash\widetilde{\Mod}_{R}}(\mathbf{1},\mathbf{1})$ in the derived category of modules with modified $q$-connection $q\dash\widetilde{\Mod}_{R}$ on $\spec(R)$.
    \end{enumerate}
\end{proposition}
