...
In the first step of the hill climbing algorithm, we select one solution $x^{*}\in S_{\mathbb{C}}$ in which we aim to decrease the $L^1$ norm $\Vert\mathrm{Im}(x^{*})\Vert_{1}$ of the vector of imaginary parts of $x^*$. The goal is to compute a step $\Delta p$ in the parameter space $\mathbb{R}^{18}$, such that the magnitude of the imaginary part of $x^{*}$ decreases as we move from $p$ to $p+\Delta p$ for some $-\varepsilon\mathbf{1}\leq\Delta p\leq \varepsilon\mathbf{1}$ where $\varepsilon$ is a small tolerance parameter and $\mathbf{1}=(1,\ldots,1)^T\in\mathbb R^{18}$ is the all-one vector.
...
Notice that this objective function considers a first order approximation of our system $F(x,p)$ at $(x^*,p)$, so we add the constraint $-\varepsilon\mathbf{1}\leq\Delta p\leq \varepsilon\mathbf{1}$ to ensure that this approximation is accurate enough.
...
\begin{align*}
    ...
    \text{subject to} &\hspace{0.2cm} - \epsilon \cdot \mathbf{1} \leq \Delta p \leq \epsilon \cdot \mathbf{1} \\
